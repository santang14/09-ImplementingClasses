...
class Point(object):
    ...
    def move_to(self, dx, dy):
        self.sum = self.sum + m.sqrt((dx - self.x) ** 2 + (dy - self.y) ** 2)
        self.x = dx
        self.y = dy
    ...
    def move_by(self, dx, dy):
        self.sum = self.sum + m.sqrt(dx ** 2 + dy ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.move = self.move + 1
        return self.sum
    ...
    def get_distance_from(self, p4):
        return m.sqrt((p4.x - self.x)**2 + (p4.y - self.y)**2)

    def get_distance_from_start(self):
        return m.sqrt((self.start_x - self.x)**2 + (self.start_y - self.y)**2)
    ...
    def halfway_to(self, p2):
        p = Point((self.x + p2.x) / 2, (self.y + p2.y) / 2)
        return p
